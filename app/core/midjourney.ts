...
export class MidjourneyApi {
  ...
  constructor() {
    ...
    this.config = {
      ...
      discordProxy:
        <string>process.env.MJ_DISCORD_PROXY || "https://discord.com",
      discordWssProxy:
        <string>process.env.MJ_DISCORD_WSS_PROXY || "wss://gateway.discord.gg",
      discordCdnProxy:
        <string>process.env.MJ_DISCORD_CDN_PROXY ||
  ...
  async submit(data: any) {
    ...
    if (data.action === "IMAGINE") {
      ...
      this.taskCall(
        taskId,
        this.client?.Imagine(prompt, this.taskLoading(taskId)),
      );
      ...
      this.taskCall(
        taskId,
        this.client?.Imagine(images.join(" "), this.taskLoading(taskId)),
      );
  ...
  async uploadImage(base64: string, toMsg: boolean = false) {
    ...
    const res = await fetch(
      `${this.config.discordProxy}/api/v9/channels/${this.config.channelId}/attachments`,
      {
        method: "POST",
        headers: {
    ...
    if (res.status != 200) {
      ...
      const uploadMsgRes = await fetch(
        `${this.config.discordProxy}/api/v9/channels/${this.config.channelId}/messages`,
        {
          method: "POST",
          headers: {
            Authorization: `${this.config.token}`,
